...
def plt_ECE(bars, num_bars, acc_bars, num_samples, ECE):
    ...
    plt.tight_layout()
    plt.legend()
    # plt.savefig(f"整体校准-{dataset_name}-{method_name}.svg", format='svg')
...
if __name__ == '__main__':
    ...
    # 计算 ECE 所需数据
    nBin = 30
    bars, num_bars, acc_bars, conf_bars, ECE = get_ECE(prob_test, pred_test, label_test, nBin)

    # 绘制 ECE　结果
    plt_ECE(bars, num_bars, acc_bars, len(label_test), ECE)
