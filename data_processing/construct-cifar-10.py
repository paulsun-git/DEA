...
dir_dataset = "../datasets/cifar-10/"

# 加载数据集
transform = transforms.Compose([transforms.ToTensor()])
train_dataset = torchvision.datasets.CIFAR10(root=dir_dataset, train=True, download=True, transform=transform)
test_dataset = torchvision.datasets.CIFAR10(root=dir_dataset, train=False, download=True, transform=transform)
...
np.savez_compressed(os.path.join(dir_save, "cifar_10-train.npz"), data=data_train, labels=labels_train)
dir_save = os.path.join(dir_dataset, "standard", "test", "01")
if not os.path.exists(dir_save):
    os.makedirs(dir_save)
np.savez_compressed(os.path.join(dir_save, "cifar_10-test.npz"), data=data_test, labels=labels_test)
dir_save = os.path.join(dir_dataset, "standard", "val", "01")
if not os.path.exists(dir_save):
    os.makedirs(dir_save)
np.savez_compressed(os.path.join(dir_save, "cifar_10-val.npz"), data=data_val, labels=labels_val)
# 设置类别采样率，构建不平衡训练集
