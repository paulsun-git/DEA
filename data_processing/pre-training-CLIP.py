...
clip_model, _, clip_preprocess = (
    open_clip.create_model_and_transforms("ViT-B-16",
                                          pretrained="E:\\Download\\CLIP\\open_clip_model.safetensors"))
...
# ---------- 3. 提取函数 ----------
def extract_clip(imgs_np, labs_np, batch_size=200):
    """
    imgs_np: (N,3,224,224) float32 0.0~1.0
    labs_np: (N,)
    return: feats_np (N,512), labs_np
    """
    N = imgs_np.shape[0]
    feats = []

    for i in tqdm(range(0, N, batch_size), desc='CLIP extract'):
        ...
        with torch.no_grad():
            # print('xb', xb.size())
            feat = clip_model.encode_image(xb)  # (B,512)
...
def get_feats_labs(dir_dataset, name_dataset):
    ...
    for batch_dir in batch_dirs:
        ...
        for file_name in file_names:
            ...
            feats, labs = extract_clip(train_X, train_Y)
...
if __name__ == '__main__':
    ...
    np.savez_compressed(os.path.join(dir_dataset, 'clip_features_labels-train.npz'), features=features, labels=labels)
