...
dir_dataset = "../datasets/spots-10/SPOTS-10-main/"
dir_results = "../datasets/spots-10/"


# 加载数据集
labels_path = os.path.join(dir_dataset, "dataset", f'train-labels-idx1-ubyte.gz')
images_path = os.path.join(dir_dataset, "dataset", f'train-images-idx3-ubyte.gz')
...
images_train, labels_train = np.array(images) / 255.0, np.array(labels)

labels_path = os.path.join(dir_dataset, "dataset", f'test-labels-idx1-ubyte.gz')
images_path = os.path.join(dir_dataset, "dataset", f'test-images-idx3-ubyte.gz')
...
images_test, labels_test = np.array(images) / 255.0, np.array(labels)

data_train = np.array(images_train, dtype=np.float32)
labels_train = np.array(labels_train, dtype=np.int32)
data_test = np.array(images_test, dtype=np.float32)
...
dir_save = os.path.join(dir_results, "standard", "train", "01")
if not os.path.exists(dir_save):
    os.makedirs(dir_save)
np.savez_compressed(os.path.join(dir_save, "spots_10-train.npz"), data=data_train, labels=labels_train)
dir_save = os.path.join(dir_results, "standard", "test", "01")
if not os.path.exists(dir_save):
    os.makedirs(dir_save)
np.savez_compressed(os.path.join(dir_save, "spots_10-test.npz"), data=data_test, labels=labels_test)
dir_save = os.path.join(dir_results, "standard", "val", "01")
if not os.path.exists(dir_save):
    os.makedirs(dir_save)
np.savez_compressed(os.path.join(dir_save, "spots_10-val.npz"), data=data_val, labels=labels_val)
# 设置类别采样率，构建不平衡训练集
