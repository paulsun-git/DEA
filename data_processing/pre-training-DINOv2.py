...
# ---------- 3. 提取函数 ----------
def extract_dino(imgs_np, labs_np, batch_size=200):
    """
    imgs_np: (N,3,224,224)  float32  0.0-1.0
    labs_np: (N, )            int/long
    return:
        feats_np: (N,384)    float32
        labs_np:  (N, )        原样返回
    """
    N = imgs_np.shape[0]
    feats = []

    for i in tqdm(range(0, N, batch_size), desc='DINOv2 extract'):
        xb = numpy_to_tensor(imgs_np[i:i+batch_size])
...
def get_feats_labs(dir_dataset, name_dataset):
    ...
    for batch_dir in batch_dirs:
        ...
        for file_name in file_names:
            ...
            feats, labs = extract_dino(train_X, train_Y)
...
if __name__ == '__main__':
    ...
    np.savez_compressed(os.path.join(dir_dataset, 'dino_features_labels-train.npz'), features=features, labels=labels)
